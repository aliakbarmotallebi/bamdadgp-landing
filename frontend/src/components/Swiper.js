...
export default function Swiper({title,data}){
    ...
    const handleNextSwip = ()=>{
        const swipPage = Math.ceil(articles?.current.scrollWidth / wrapper?.current.clientWidth);
        if(swipPage > swip){
    ...
    const handlePrevSwip = ()=>{
        const swipPage = Math.ceil(articles?.current.scrollWidth / wrapper?.current.clientWidth);
        if(swip > 1){
            setLeft((wrapper?.current.clientWidth * (swip-1)) - wrapper?.current.clientWidth)
            setSwip(swip-1);
        }
    }
    ...
    return (
        <section className="my-4 w-full">
                ...
                <div className="font-semibold text-xl text-orange-900">
                    {title}
                </div>
                ...
                {
                    w && w > 0?
                        <div ref={articles} style={{left:left+'px'}} className="flex relative items-center transition-[left] duration-500 ease-in">
                        {
                            data.map((item,index)=>(
                                <article
                                key={index}
                                ...
                                >
                                    ...
                                    src={process.env.NEXT_PUBLIC_BASE_URL+item.product_image.url}
                    ...
                    <div className="grid min-h-[140px] w-full place-items-center overflow-x-scroll rounded-lg p-6 lg:overflow-visible">
                        <div className="flex items-end gap-8">
                            <svg className="w-12 h-12 text-gray-200 animate-spin" viewBox="0 0 64 64" fill="none"
                            ...
                            <path
                                ...
                                stroke="currentColor" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round" className="text-yellow-500">
                            </path>
                            </svg>
                        </div>
                    </div>
