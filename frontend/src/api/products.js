...
const getSingleProduct = async slug => {
  try {
    const response = await axios.get(
      `${API_URL}/products?populate=*&filters[product_slug]=${slug}`
    )
    return handleApiResponse(response, 'product')
  } catch (error) {
    throw error
...
const getSpecesProduct = async id => {
  try {
    const response = await axios.get(
      `${API_URL}/products?populate[speces][populate]=*&filters[id]=${id}`
    )
    return handleApiResponse(response, 'product')
  } catch (error) {
    throw error
...
const getCommentsProduct = async id => {
  try {
    const response = await axios.get(
      `${API_URL}/products?populate[comments][filters][comment_status]=publish&filters[id]=${id}`
    )
    return handleApiResponse(response, 'product')
  } catch (error) {
    throw error
